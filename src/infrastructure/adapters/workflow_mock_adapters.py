...
class TaskMockAdapter(BaseMockAdapter):
    ...
    def execute(self, workflow: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Build task payload using step_action (not workflow title)
            payload = {
                'task_id': f"TASK_{uuid.uuid4().hex[:10].upper()}",
                'assignee': parameters.get('assignee', self._determine_assignee(workflow_type, step_action)),
                'title': parameters.get('title', self._generate_title(step_action)),
                'description': parameters.get('description', self._generate_description(step_action, step_details)),
                'priority': parameters.get('priority', self._determine_priority(step_action)),
                'due_date': parameters.get('due_date', self._calculate_due_date(step_action)),
                'category': self._determine_category(step_action),
                'workflow_id': workflow.get('id'),
                'customer_id': f"CUST_{uuid.uuid4().hex[:8]}",
                'estimated_duration': self._estimate_duration(step_action),
                'task_type': self._determine_task_type(step_action),
                'status': 'assigned',
                'created_at': datetime.now(timezone.utc).isoformat(),
                'created_by': 'workflow_executor',
                'dependencies': [],
                'tags': self._generate_tags(step_action),
                'checklist': self._generate_checklist(step_action),
                'automation_rules': {
                    'auto_remind': True,
    ...
    def _determine_assignee(self, workflow_type: str, action_item: str) -> str:
        """Determine task assignee based on workflow type and step action."""
    ...
    def _generate_title(self, action_item: str) -> str:
        """Generate clear, actionable task title from step action."""
    ...
    def _generate_description(self, action_item: str, details: str) -> str:
        """Generate detailed task description from step action and details."""
        base_description = f"Task: {action_item}\n\n"

        if details:
            base_description += f"Details: {details}\n\n"
...
- Task marked complete with detailed notes"""
    ...
    def _determine_priority(self, action_item: str) -> str:
        """Determine task priority based on step action."""
        action_lower = action_item.lower()
    ...
    def _calculate_due_date(self, action_item: str) -> str:
        """Calculate appropriate due date based on step action."""
        action_lower = action_item.lower()
    ...
    def _determine_category(self, action_item: str) -> str:
        """Determine task category based on step action."""
        action_lower = action_item.lower()
    ...
    def _determine_task_type(self, action_item: str) -> str:
        """Determine if task is manual, automated, or requires approval based on step action."""
        action_lower = action_item.lower()
    ...
    def _estimate_duration(self, action_item: str) -> str:
        """Estimate task duration based on step action."""
        action_lower = action_item.lower()
    ...
    def _generate_tags(self, action_item: str) -> list:
        """Generate relevant tags for task based on step action."""
    ...
    def _generate_checklist(self, action_item: str) -> list:
        """Generate task checklist based on step action."""
        base_checklist = [

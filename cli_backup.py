...
def list_analyses(
    ...
    REST API: GET /api/v1/analyses
    
    Examples:
      cli.py list-analyses
      cli.py list-analyses --limit 10
    """
    try:
        client = get_client()
        ...
        # Display analyses in table format
...
def analyze(
    ...
    try:
        ...
        if all_transcripts:
            ...
            # Display comprehensive analysis results - all available data
            ...
            # Risk Assessment Section (always show if available)
            ...
            advisor_metrics = result.get('advisor_metrics', {})
            if advisor_metrics:
                console.print(f"\n👨‍💼 [bold green]Advisor Performance:[/bold green]")
                if 'empathy_score' in advisor_metrics:
                    console.print(f"   Empathy Score: {advisor_metrics['empathy_score']:.2%}")
                if 'compliance_adherence' in advisor_metrics:
                    console.print(f"   Compliance Adherence: {advisor_metrics['compliance_adherence']:.2%}")
                if 'solution_effectiveness' in advisor_metrics:
                    console.print(f"   Solution Effectiveness: {advisor_metrics['solution_effectiveness']:.2%}")
                
                coaching_opportunities = advisor_metrics.get('coaching_opportunities', [])
                if coaching_opportunities:
                    console.print(f"   Coaching Opportunities:")
                    for opportunity in coaching_opportunities:
                        console.print(f"     • {opportunity}")
            
            # Topics and Concerns Section
            topics_discussed = result.get('topics_discussed', [])
            if topics_discussed:
                console.print(f"\n💬 [bold cyan]Topics Discussed:[/bold cyan]")
                for topic in topics_discussed:
                    console.print(f"   • {topic}")
            
            payment_concerns = result.get('payment_concerns', [])
            if payment_concerns:
                console.print(f"\n💰 [bold yellow]Payment Concerns:[/bold yellow]")
                for concern in payment_concerns:
                    console.print(f"   • {concern}")
            
            product_opportunities = result.get('product_opportunities', [])
            if product_opportunities:
                console.print(f"\n🎯 [bold green]Product Opportunities:[/bold green]")
            ...
            required_disclosures = result.get('required_disclosures', [])
            
            if compliance_flags or required_disclosures:
                console.print(f"\n📋 [bold red]Compliance:[/bold red]")
                if compliance_flags:
                    console.print(f"   Flags: {len(compliance_flags)} found")
                    for flag in compliance_flags:
                        console.print(f"     • {flag}")
                else:
                    console.print(f"   Flags: None")
                
                if required_disclosures:
                    console.print(f"   Required Disclosures:")
                    for disclosure in required_disclosures:
                        console.print(f"     • {disclosure}")
                else:
                    console.print(f"   Required Disclosures: None")
...
def transcript_create(
    ...
    """Create new transcript - replaces generate command."""
    try:
        client = get_client()
...
def transcript_list(
    ...
    """List all transcripts - replaces list command."""
    try:
        client = get_client()
...
def transcript_get(transcript_id: str):
    """Get specific transcript details - replaces get command."""
    try:
        client = get_client()
...
def analysis_create(
    ...
    """Create analysis for transcript - replaces analyze command."""
    try:
        client = get_client()
        ...
        # Display comprehensive analysis results (reuse from analyze command)
        analysis_id = result.get('analysis_id')
        intent = result.get('primary_intent')
        
        console.print(f"\n📄 [bold green]Analysis Results[/bold green]:")
        console.print(f"   Analysis ID: [cyan]{analysis_id}[/cyan]")
        console.print(f"   Transcript ID: [cyan]{transcript}[/cyan]")
        console.print(f"   Intent: {intent}")
...
def analysis_list(
    ...
    """List all analyses - replaces list-analyses command."""
    try:
        client = get_client()
